...
O arquivo `.env` já é ignorado pelo Git. Em produção, o backend deve ser criado como um serviço separado no Railway usando `railway.backend.toml`; o serviço atual do frontend continua usando `railway.web.toml`.

## Rodar localmente

```bash
pnpm install
pnpm --filter @vozes/api dev
```
